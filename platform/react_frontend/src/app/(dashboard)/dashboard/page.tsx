...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const loadHealth = async () => {
            try {
                ...
                setHealthError(error instanceof Error ? error.message : "Failed to load monitoring data");
            }
            setLoading(false);
        };
        loadHealth();
    ...
    const performances = health ? Object.values(health.agent_performances || {}) : [];
    const performanceEntries = health ? Object.entries(health.agent_performances || {}) : [];

    const avgWinRate = performances.length
        ? ((performances.reduce((sum, p) => sum + p.win_rate, 0) / performances.length) * 100).toFixed(1)
        : "0.0";

    const totalSignals = performances.reduce((sum, p) => sum + p.total_signals, 0);
                ...
                {healthError && (
                    <Card className="border-amber-500/30 bg-amber-500/10">
                        <CardContent className="p-4 text-sm text-amber-700 dark:text-amber-300">
                            Monitoring data is temporarily unavailable: {healthError}
                        </CardContent>
                    </Card>
                )}
                    ...
                    <Card className="border-border/50 bg-card/80 backdrop-blur">
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                {performanceEntries.map(([type, perf]) => (
                                    <div
                                        key={type}
