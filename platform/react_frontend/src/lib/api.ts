...
async function fetcher<T>(url: string): Promise<T> {
    try {
        const res = await fetchWithTimeout(`${API_BASE}${url}`, { cache: "no-store" }, 30000);
        if (!res.ok) {
        ...
        if (isAbortLikeError(error)) {
            throw new ApiRequestError(
                "Backend request timed out. Please retry in a few seconds.",
            );
        }

        throw new ApiRequestError(
            "Unable to reach backend API. Verify the backend is running on port 8000.",
        );
    }
}
