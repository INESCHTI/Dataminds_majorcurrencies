import MetaTrader5 as mt5
from datetime import datetime, timedelta
import pandas as pd
from influxdb_client import InfluxDBClient, Point, WritePrecision
from influxdb_client.client.write_api import SYNCHRONOUS
import os
import sys
from dotenv import load_dotenv

# Force UTF-8 encoding for Windows console (to support emojis)
if sys.stdout.encoding != 'utf-8':
    sys.stdout.reconfigure(encoding='utf-8')

script_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(script_dir)
dotenv_path = os.path.join(project_root, 'config', '.env')
load_dotenv(dotenv_path=dotenv_path, override=True)

MT5_LOGIN    = int(os.getenv("MT5_LOGIN"))
MT5_PASSWORD = os.getenv("MT5_PASSWORD")
MT5_SERVER   = os.getenv("MT5_SERVER")

INFLUXDB_URL    = os.getenv("INFLUXDB_URL")
INFLUXDB_TOKEN  = os.getenv("INFLUXDB_TOKEN")
INFLUXDB_ORG    = os.getenv("INFLUXDB_ORG")
INFLUXDB_BUCKET = os.getenv("INFLUXDB_BUCKET")

PAIRS = ["EURUSD", "USDJPY", "GBPUSD", "USDCHF"]
TIMEFRAMES = {
    "1H": mt5.TIMEFRAME_H1,
    "4H": mt5.TIMEFRAME_H4,
    "1D": mt5.TIMEFRAME_D1,
}
DAYS_BACK = 365 * 5
MT5_PATH = r"C:\Program Files\MetaTrader 5\terminal64.exe"  # ← FIXED

def connect_mt5():
    if not mt5.initialize(path=MT5_PATH):  # ← FIXED
        print("❌ MT5 initialization failed:", mt5.last_error())
        return False
    if not mt5.login(MT5_LOGIN, MT5_PASSWORD, MT5_SERVER):
        print("❌ MT5 login failed:", mt5.last_error())
        mt5.shutdown()
        return False
    acc = mt5.account_info()
    print("✅ Connected to MT5")
    print(f"   Account: {acc.login}")
    print(f"   Server : {MT5_SERVER}")
    return True

def get_historical_data(symbol, tf_name, tf_mt5):
    print(f"\n📊 Fetching {symbol} - {tf_name}")
    end_date   = datetime.now()
    start_date = end_date - timedelta(days=DAYS_BACK)
    rates = mt5.copy_rates_range(symbol, tf_mt5, start_date, end_date)
    if rates is None or len(rates) == 0:
        print(f"   ❌ No data returned")
        return None
    df = pd.DataFrame(rates)
    df["time"] = pd.to_datetime(df["time"], unit="s")
    print(f"   ✅ Candles: {len(df):,}")
    print(f"   📅 Range : {df['time'].min()} → {df['time'].max()}")
    return df

def write_to_influx(df, symbol, timeframe):
    print("   💾 Writing to InfluxDB...")
    client = InfluxDBClient(url=INFLUXDB_URL, token=INFLUXDB_TOKEN, org=INFLUXDB_ORG)
    write_api = client.write_api(write_options=SYNCHRONOUS)
    points = []
    for _, row in df.iterrows():
        point = (
            Point("forex_prices")
            .tag("symbol", symbol)
            .tag("timeframe", timeframe)
            .field("open",   float(row["open"]))
            .field("high",   float(row["high"]))
            .field("low",    float(row["low"]))
            .field("close",  float(row["close"]))
            .field("volume", int(row["tick_volume"]))
            .time(row["time"], WritePrecision.NS)
        )
        points.append(point)
    try:
        write_api.write(bucket=INFLUXDB_BUCKET, org=INFLUXDB_ORG, record=points)
        print(f"   ✅ {len(points):,} points written")
    except Exception as e:
        print("   ❌ InfluxDB write error:", e)
    finally:
        client.close()

def main():
    print("=" * 60)
    print("📈 MT5 → INFLUXDB DATA ACQUISITION")
    print("=" * 60)
    if not connect_mt5():
        return
    total = 0
    for symbol in PAIRS:
        for tf_name, tf_mt5 in TIMEFRAMES.items():
            df = get_historical_data(symbol, tf_name, tf_mt5)
            if df is not None:
                write_to_influx(df, symbol, tf_name)
                total += len(df)
    mt5.shutdown()
    print("\n" + "=" * 60)
    print("✅ DATA ACQUISITION COMPLETE")
    print(f"📦 Total candles written: {total:,}")
    print("=" * 60)

if __name__ == "__main__":
    main()